...
void writeTPCF_DM(ConfigGetModel para, error **err){
  ...
  */
  ...
  fprintf(fileOut, "#r[h^-1 Mpc]  xi_dm(r)  wp_dm(r)[h^-1 Mpc]  k[h Mpc^-1]  P(k)[(h Mpc^-1)^3]\n"); 
  for(i=0;i<para.nbins;i++) {
    fprintf(fileOut,"%f %e %f %f %e\n", r[i], xi_dm[i], wp_dm[i], k[i], pk_dm[i]);
  }
